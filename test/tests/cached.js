...
if (!process.env.SOCKET_SERVICE_URL) {
  console.log(
    "Please set SOCKET_SERVICE_URL environment variable to test cached-behaviour"
  );
} else {
  let ids, newHenk;
  ...
  describe("Connector", function () {
    ...
    describe("metadata handling", () => {
      ...
      it("does not break on a regular result-object containing a property metadata (e.g. File)", (done) => {
        cbc.getId("File", { metadata: { $exists: true } }).then(
          (fileId) =>
            cbc.getById("File", fileId).then(
              (file) => {
